...
export class NeptunePriceClient {
    ...
    private async fetchFromCoinGecko(): Promise<NeptunePriceData> {
        ...
        const data = response["neptune-cash"] as Record<string, unknown>;

        return {
            symbol: "NEPTUNE",
            price: Number(data.usd) || 0,
            change24h: Number(data.usd_24h_change) || 0,
            changePercent24h: Number(data.usd_24h_change) || 0,
            volume24h: Number(data.usd_24h_vol) || 0,
            marketCap: Number(data.usd_market_cap) || 0,
            high24h: Number(data.usd_24h_high) || 0,
            low24h: Number(data.usd_24h_low) || 0,
